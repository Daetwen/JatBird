...
public class Main extends Application {
     ...
     */
    public static Person person = new Person(300);
    /**
    ...
    /**
     * Счётчик монет собрано
     */
    public static int coin_score = 0;
    /**
     * Счётчик бонусов собрано
     */
     ...
     */
    String[] SaveText = saveinfo.SaveMessage();
    /**
    ...
    double AveragePositionX = 100;
    public ArrayList<String> map = new ArrayList<>();
    MapOfGame mog = new MapOfGame();
        ...
        try {
            ...
            scene.setOnKeyPressed(key -> {
                ...
                if (keyCode.equals(KeyCode.R)) {
                    saveinfo.Reload("Save.txt");
                    isSaved = true;
                    return;
                }
                if (keyCode.equals(KeyCode.ESCAPE)) {
                    IsDeadMenu(primaryStage, menu, true);
                    timer.stop();
                    restart();
                    return;
                }
            ...
            timer = new AnimationTimer() {
                ...
                public void handle(long now) {
                    ...
                    if (person.count == true && isSaved == false) {
                        IsDeadMenu(primaryStage, menu, person.count);
                        timer.stop();
                        mog.SetMapOfGame(person.getTranslateY(), person.getTranslateX(), coin_score, bonus_score);
                        saveinfo.IsRecord();
                        restart();
                    }
                }
    ...
    public void IsESCMenu(Stage primaryStage, Scene menu, boolean state) {
        try {
            StopMenu(score, coin_score, 900, 600);
            MenuItem mainmenu = new MenuItem("ВЕРНУТЬСЯ В ГЛАВНОЕ МЕНЮ", 450, 40);
            MenuItem continye = new MenuItem("ПРОДОЛЖИТЬ", 450, 40);
            SubMenu localmenu = new SubMenu(225, 290, mainmenu, continye);
            ...
            mainmenu.setOnMouseClicked(event -> {
                ...
                timer.stop();
                restart();
                SaveText = saveinfo.SaveMessage();
            });
            continye.setOnMouseClicked(event -> {
     ...
     */
    public void IsDeadMenu(Stage primaryStage, Scene menu, boolean state) {
        try {
            StopMenu(score, coin_score, 900, 600);
            MenuItem mainmenu = new MenuItem("ВЕРНУТЬСЯ В ГЛАВНОЕ МЕНЮ", 450, 40);
    ...
    public void start(Stage primaryStage) throws Exception {
        try {
            ...
            MenuItem casee = new MenuItem("КАРТА ИГРЫ", 250, 40);
            ...
            records.setOnMouseClicked(event -> {
                ...
                save1.setOnMouseClicked(event1 -> {
                    saveinfo.StartFromRecords(0);
                    Play(primaryStage, menu, ComplexityX, ComplexityY);
                });
                save10.setOnMouseClicked(event1 -> {
                    saveinfo.StartFromRecords(9);
                    Play(primaryStage, menu, ComplexityX, ComplexityY);
                });
                //SortMenu(menu_pane);
            });
            saveback.setOnMouseClicked((event -> menuBox.setSubMenu(mainMenu)));
            casee.setOnMouseClicked(event -> {
                MapOfGame mog = new MapOfGame();
                map = mog.SaveMessage(map);
                MenuItem info0 = new MenuItem(map.get(0), 580, 40, Pos.CENTER_LEFT);
                MenuItem info1 = new MenuItem(map.get(1), 580, 40, Pos.CENTER_LEFT);
                MenuItem info2 = new MenuItem(map.get(2), 580, 40, Pos.CENTER_LEFT);
                MenuItem info3 = new MenuItem(map.get(3), 580, 40, Pos.CENTER_LEFT);
                MenuItem info4 = new MenuItem(map.get(4), 580, 40, Pos.CENTER_LEFT);
                MenuItem info5 = new MenuItem(map.get(5), 580, 40, Pos.CENTER_LEFT);
                SubMenu MapMenu = new SubMenu(10, 285, info0, info1, info2, info3, info4, info5, saveback);
                menuBox.setSubMenu(MapMenu);
            });
...
//    public void SortMenu(Stage primaryStage,Scene menu ){
//        MenuItem firstsort = new MenuItem("По уровню сложности",190,40);
//        MenuItem secondsort = new MenuItem("По очкам",190,40);
//        MenuItem thirdsort = new MenuItem("По накопленным монетам",190,40);
//        firstsort.setTranslateX(10);
//        firstsort.setTranslateY(102);
//        secondsort.setTranslateX(205);
//        secondsort.setTranslateY(102);
//        thirdsort.setTranslateX(400);
//        thirdsort.setTranslateY(102);
//        //menu.getChildren().addAll(firstsort,secondsort,thirdsort);
//    }
...
//        saveinfo.Read("Save2.txt",saves2);
//        saves2.add(save);
...
//        saveinfo.Write("Save2.txt",saves2);
//        saveinfo.SaveMessage();
//    }
//
//    /**
